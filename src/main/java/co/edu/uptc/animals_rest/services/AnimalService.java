...
public class AnimalService {
    ...
    public List<Animal> getAnimalByCategory(String category) throws IOException {
        List<String> listAnimal = Files.readAllLines(Paths.get(filePath));
        List<Animal> animales = new ArrayList<>();
        ...
        for (String line : listAnimal) {
            ...
            if (parts.length == 2) {
                String categoria = parts[0].trim();
                String nombre = parts[1].trim();
                if (categoria.equalsIgnoreCase(category)) {
                    animales.add(new Animal(nombre, categoria));
                }
            }
        }
    ...
    public List<Animal> getAnimalByNameLength(int length) throws IOException {
        List<String> listAnimal = Files.readAllLines(Paths.get(filePath));
        List<Animal> animales = new ArrayList<>();
